...
namespace {
    ...
    void TestDiagnostic() {
        ...
        std::cout << "holds double: " << std::holds_alternative<double>(value) << "\n";
        std::cout << "holds string: " << std::holds_alternative<std::string>(value) << "\n";
        std::cout << "holds error: " << std::holds_alternative<FormulaError>(value) << "\n";
    }
    ...
    void TestDiagnostic2() {
        auto sheet = CreateSheet();

        // Сценарий 1: цепочка ссылок A->B->C
        sheet->SetCell("A1"_pos, "1");
        sheet->SetCell("B1"_pos, "=A1");
        sheet->SetCell("C1"_pos, "=B1");
        auto v1 = sheet->GetCell("C1"_pos)->GetValue();
        std::cerr << "Цепочка A->B->C: index=" << v1.index() << " (ожидаем 1=double)\n";

        // Сценарий 2: ПОВТОРНЫЙ SetCell поверх существующей формулы
        sheet->SetCell("D1"_pos, "=A1");
        sheet->SetCell("D1"_pos, "=A1");  // ещё раз, та же формула
        auto v2 = sheet->GetCell("D1"_pos)->GetValue();
        std::cerr << "Повторный SetCell: index=" << v2.index() << "\n";

        // Сценарий 3: через CONST GetCell
        const SheetInterface& constSheet = *sheet;
        auto v3 = constSheet.GetCell("B1"_pos)->GetValue();
        std::cerr << "Через const GetCell: index=" << v3.index() << "\n";

        // Сценарий 4: два разных SetCell на одну ссылку туда-сюда
        sheet->SetCell("E1"_pos, "=A1");
        sheet->SetCell("E1"_pos, "text");
        sheet->SetCell("E1"_pos, "=A1");
        auto v4 = sheet->GetCell("E1"_pos)->GetValue();
        std::cerr << "После смены формула->текст->формула: index=" << v4.index() << "\n";
    }
}  // namespace

int main() {
    TestRunner tr;
    RUN_TEST(tr, TestDiagnostic1);
    RUN_TEST(tr, TestDiagnostic2);
